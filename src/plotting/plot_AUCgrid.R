...
# comparison conditions (EZ/Robust x Clean/Contaminated) when plot_by = "beta",
# or beta curves overlaid across conditions when plot_by = "condition".
plot_AUC_nested_2x2 <- function(main_dir, output_dir, plot_by = "condition", y_range = NULL,
  ...
  base_name <- paste0("AUC_2x2_by_", plot_by, "_T", t_level, suffix)
  ...
  # Helper to render a single device; keeps layout consistent
  render_2x2 <- function(open_device) {
    open_device()
      ...
      for (boundary_idx in seq_along(boundary_levels)) {
        ...
        if (length(auc_data_list) > 0) {
          ...
          if (plot_by == "condition") {
            ...
            show_legend <- (drift_idx == 2) && (boundary_idx == 2)
            plot_cell_by_condition(auc_df, conditions, condition_labels, y_range,
            ...
            plot_cell_by_beta(auc_df, conditions, condition_labels, y_range,
                              show_x_axis, show_y_axis, show_legend,
                              highlight_cell = FALSE, legend_position = "bottomright",
                              legend_cex = 1.55, y_cex = 1.5, x_cex = 1.7)
          }
        } else {
          plot.new()
        }
        
        # In-panel T label (top-left, consistent with nested aesthetic)
        # if ((drift_idx == 1) && (boundary_idx == 1)) {
        #  usr <- par("usr")
        #  text(x = usr[2] - 0.03 * diff(usr[1:2]), y = usr[3] + 0.05 * diff(usr[3:4]),
        #       labels = paste("T =", t_level), adj = c(1, 0), cex = 2.5, font = 2, col = "black")
        # }
      }
    }
    ...
    tryCatch({
      ...
      mtext(expression(paste("Effect size (", beta, ")")), side = 1, line = 3.8, cex = 2.2, outer = TRUE, font = 1)
...
plot_cell_by_beta <- function(auc_df, conditions, condition_labels, y_range, show_x_axis, 
  ...
  if (show_legend) {
    legend(legend_position,
           ...
           lwd = 2, pch = 19, bty = "n", cex = legend_cex)
  }
}
...
plot_AUC_nested_by_param <- function(main_dir, output_dir, t_level_select = 40, beta_level_select = 0.2,
                                     x_param = "drift", custom_title_label = NULL, run_diagnose = FALSE) {
  if (!x_param %in% c("drift", "bound")) {
    stop("Invalid x_param. Use 'drift' or 'bound'.")
  }
  ...
  bin_spec <- get_param_bin_spec(x_param)
  ...
  par(mfrow = c(1, 2), oma = c(3.5, 3.4, 2.3, 0.5), mar = c(2.4, 2.4, 1.4, 0.15), cex = 1.0)
  
  for (i in seq_along(fixed_levels)) {
    panel_key <- fixed_levels[i]
    show_y_axis <- (i == 1)
    show_legend <- (i == 1)
    plot_auc_param_cell(panel_df = panel_data[[panel_key]], conditions = conditions,
  ...
  mtext(panel_titles[1], side = 3, line = -1, at = 0.25, outer = TRUE, cex = 1.8, font = 2)
  mtext(panel_titles[2], side = 3, line = -1, at = 0.75, outer = TRUE, cex = 1.8, font = 2)
  mtext("Area Under Curve (AUC)", side = 2, line = 1.7, outer = TRUE, cex = 1.8, font = 2)
  mtext(x_axis_label, side = 1, line = 1.5, outer = TRUE, cex = 1.8, font = 2)
  dev.off()
  
  cat("AUC nested-by-parameter plot saved to:", output_path, "\n")
}

# Define fixed parameter bins based on simulation design.
get_param_bin_spec <- function(x_param) {
  if (identical(x_param, "drift")) {
...
# Extract multiple (x_bin_center, auc) points from one file at selected beta level.
extract_auc_param_by_bins <- function(file_path, beta_level_select, x_col, x_level, bin_spec, run_diagnose = FALSE) {
  ...
  for (b in seq_len(length(breaks) - 1)) {
    ...
    if (!any(idx_alt)) {
      nearest_beta <- unique(true_betas[in_bin])
      nearest_beta <- nearest_beta[!is.na(nearest_beta)]
      if (length(nearest_beta) > 0) {
        beta_use <- nearest_beta[which.min(abs(nearest_beta - beta_level_select))]
        idx_alt <- in_bin & (abs(true_betas - beta_use) < 1e-8)
      }
    }
...
# Plot one panel for the nested-by-parameter AUC function.
plot_auc_param_cell <- function(panel_df, conditions, condition_labels, x_range, y_range, x_ticks, y_ticks,
  ...
  for (i in seq_along(conditions)) {
    ...
    for (lev in unique(dd$x_level)) {
      ...
      points(dd_lev$x_value[ord], dd_lev$auc[ord], col = condition_colors[i], pch = points[i], cex = 1.8)
    }
  }
  
  if (show_legend) {
    legend(legend_position, legend = condition_labels, col = condition_colors,
           lwd = 2, pch = points, bty = "n", cex = 1.2)
  }
}